...
def get_all_info_by_country(country_id: int):
    result = []
    new_url = f"{API_BASE_URL}/poi?{'&'.join(f'{key}={value}' for key, value in URL_PARAMS.items())}&countryid={country_id}&connectiontypeid=2, 32, 33"
    response = requests.get(new_url)
    response.raise_for_status()
    data = response.json()
    for station in data:
...
def all_stations_info():
    ...
    for country_id in range(251):
        ...
        stations.append(country_stations)
    return stations
